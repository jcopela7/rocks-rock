...
final class APIClient {
    ...
    func deleteAscent(_ id: UUID) async throws -> AscentDTO {
        let env: APIListEnvelope<AscentDTO> = try await helpers.delete("ascent/\(id.uuidString)", token: try requireToken())
        return env.data
    }

    func listLocations() async throws -> [LocationDTO] {
        let env: APIListEnvelope<[LocationDTO]> = try await helpers.get("location", token: try requireToken())
        return env.data
    }

    func getCountOfAscentsGroupByLocation() async throws -> [CountOfAscentsByLocationDTO] {
        let env: APIListEnvelope<[CountOfAscentsByLocationDTO]> = try await helpers.get("ascent/count/location", token: try requireToken())
        return env.data
    }
