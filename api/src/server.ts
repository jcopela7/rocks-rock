...
app.get("/db/health", async () => {
  ...
  return { ok: true, version: (version as unknown as { rows: { version: string }[] }).rows?.[0]?.version ?? "unknown" };
});
...
app.setErrorHandler((err, req, reply) => {
  ...
  const pg: { detail?: string, code?: string, constraint?: string, message?: string } = err?.cause ?? err;
  req.log.error(pg);
