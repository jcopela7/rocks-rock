...
struct ClimbingCalendarView: View {
  ...
  private var weekStreak: Int {
    ...
    let weeksWithClimbs = Set(
      ascents.map { ascent in
        let components = calendar.dateComponents(
          [.yearForWeekOfYear, .weekOfYear], from: ascent.climbedAt)
    ...
    for i in 1..<weekDates.count {
      ...
      if let expectedPreviousWeek = calendar.date(
        byAdding: .weekOfYear, value: -1, to: weekDates[i]),
    ...
    VStack(alignment: .leading, spacing: 12) {
      ...
      LazyVGrid(columns: Array(repeating: GridItem(.flexible(), spacing: 4), count: 7), spacing: 4)
      {
        ...
        ForEach(1...daysInMonth, id: \.self) { day in
          ...
          let dateString =
            "\(dayComponents.year ?? 0)-\(dayComponents.month ?? 0)-\(dayComponents.day ?? 0)"
